...
router.get('/', function (req, res) {
    ...
    if (util.allDefined(config)) {
        var botConfigs = Object.keys(config.bot);
        var geoConfigs = Object.keys(config.geocoder);


        htmlString = '<h1>Successful Set-up</h1><h3>Please verify settings below:</h3><table><thead><tr><th>Configuration</th><th>Value</th></tr>';

        for (var botIndex = 0; botIndex < botConfigs.length; botIndex++) {
            htmlString += '<tr><td>' + botConfigs[botIndex] + '</td><td>' + config.bot[botConfigs[botIndex]] + '</td></tr>';
        }

        htmlString += '<tr><th>Geocoder Configurations </th></tr>';
        for (var geoIndex = 0; geoIndex < geoConfigs.length; geoIndex++) {
            htmlString += '<tr><td>' + geoConfigs[geoIndex] + '</td><td>' + config.geocoder[geoConfigs[geoIndex]] + '</td></tr>';

            if (geoIndex === geoConfigs.length - 1) {
